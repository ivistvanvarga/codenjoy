...
class Testsolver(unittest.TestCase):
    ...
    def test_applicable_act_with_a_bomb(self):
        test = BombermanBoard(board_string=Testsolver.board,parrent=None,operator=Direction('ACT'),deep=0)
        self.assertTrue(Direction('ACT') not in test.getOperators())
        self.assertTrue(len(test.getOperators())>0)
    ...
    def test_treeSource_two_step2(self):
        ...
        suggest = TreeSerch(state=test1,deep=1,findAll=True).serch()
        suggest_state = suggest.pop()
        self.assertTrue(suggest_state.getOperator() in test1.getOperators())
    ...
    def test_treeSource_getpath2(self):
        ...
        #print(test1)
        #print(suggest_state)
        self.assertTrue(Direction('UP') in TreeSerch.getPath(node=suggest_state))
    
    def test_treeSource_with_a_bomb(self):
        test1 = BombermanBoard(board_string=Testsolver.board,parrent=None,operator=Direction('ACT'),deep=0)
        suggest = TreeSerch(state=test1,deep=2,findAll=True).serch()
        suggest_state = suggest.pop()
        print(test1)
        print(suggest_state)
